...
// Key bindings, notes to keyCodes.
	...
	// Generate keyboard
	let visualKeyboard1 = document.getElementById('keyboard1');
	let selectSound = {
		value: "0" //piano
	};
	...
	for(var i=-2;i<=1;i++) {
		for(var n in notes) {
			if(n[2]!='b') {
				...
				thisKey.setAttribute('ID', 'KEY_' + n + ',' + i);
				thisKey.addEventListener(evtListener[0], (function(_temp) { return function() { fnPlayKeyboard({keyCode:_temp}); } })(reverseLookup[n + ',' + i]));
				visualKeyboard1[n + ',' + i] = thisKey;
				visualKeyboard1.appendChild(thisKey);
	...
	visualKeyboard1.style.width = iWhite * 3 + 'rem';
...
// Detect keypresses, play notes.
	...
	var fnPlayKeyboard = function(e) {
		...
		if(keyboard[e.keyCode]) {
			if(visualKeyboard1[keyboard[e.keyCode]]) {
				visualKeyboard1[keyboard[e.keyCode]].style.backgroundColor = pressColor;
				//visualKeyboard1[keyboard[e.keyCode]].classList.add('playing'); //adding class only affects keypress and not mouse click
				visualKeyboard1[keyboard[e.keyCode]].style.marginTop = '5px';
				visualKeyboard1[keyboard[e.keyCode]].style.boxShadow = 'none';
			}
			var arrPlayNote = keyboard[e.keyCode].split(',');
			var note = arrPlayNote[0];
			var octaveModifier = arrPlayNote[1]|0;
	...
	var fnRemoveKeyBinding = function(e) {
		...
		while(i--) {
			if(keysPressed[i]==e.keyCode) {
				if(visualKeyboard1[keyboard[e.keyCode]]) {
					//visualKeyboard1[keyboard[e.keyCode]].classList.remove('playing');
					visualKeyboard1[keyboard[e.keyCode]].style.backgroundColor = '';
					visualKeyboard1[keyboard[e.keyCode]].style.marginTop = '';
					visualKeyboard1[keyboard[e.keyCode]].style.boxShadow = '';
				}
				keysPressed.splice(i, 1);
			}
		}
